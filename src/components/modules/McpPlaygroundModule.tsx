            ...
            {MCP_TOOL_DEFINITIONS.map((def) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-[11px] truncate mt-0.5 ${isSelected ? "text-slate-300" : "text-slate-500 dark:text-slate-400"}`}>
                    {def.description}
                  </div>
                </button>
              );
        ...
        <div className="lg:col-span-8 space-y-3">
          ...
          <div className="border border-[#CBD5E1] dark:border-slate-700 bg-white dark:bg-slate-800 rounded-sm p-3.5 space-y-2 shadow-xs">
            <div className="flex items-center justify-between border-b border-[#E2E8F0] dark:border-slate-700 pb-2">
              <div>
                <h4 className="text-xs font-bold font-mono text-slate-900 dark:text-slate-100">{activeDef?.name}</h4>
                <p className="text-xs text-slate-600 dark:text-slate-400 mt-0.5">{activeDef?.description}</p>
              </div>
              <button
          ...
          <div className="border border-[#CBD5E1] dark:border-slate-700 bg-white dark:bg-slate-800 rounded-sm overflow-hidden shadow-xs">
            <div className="px-3.5 py-2 bg-[#F8FAFC] dark:bg-slate-900/50 border-b border-[#E2E8F0] dark:border-slate-700 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Code size={14} className="text-slate-600 dark:text-slate-400" />
              ...
              {executionResult && (
                <button
                  onClick={copyResult}
                  className="flex items-center gap-1 text-[11px] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-800 border border-[#CBD5E1] dark:border-slate-700 px-2 py-0.5 rounded-sm"
                >
